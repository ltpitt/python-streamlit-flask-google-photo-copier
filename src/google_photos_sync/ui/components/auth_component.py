...
def render_auth_section(account_type: str, session_key: str) -> None:
    ...
    if session_key not in st.session_state or not st.session_state[session_key]:
        # Session state empty, check if credentials exist on server
        ...
        if st.button(
            ...
            with st.spinner(t("auth.initiating_oauth")):
                ...
                if result:
                    ...
                    # Provide clickable link and auto-open browser
                    st.markdown(
                    ...
                    # Input for authorization code (simplified flow)
                    with st.form(key=f"auth_code_form_{session_key}"):
                        ...
                        if submitted and email and auth_code:
                            # Exchange code for credentials via API
                            try:
                                callback_url = (
                                    f"{_get_api_base_url()}/api/auth/callback"
                                )
                                callback_params = {
                                    "code": auth_code,
                                    "state": state,
                                ...
                                st.error(
                                    t("auth.api_connection_error", error=str(e))
                                )
